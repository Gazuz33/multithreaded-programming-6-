...
// Запускаем поток
    ...
    public void getBouncingBall(BouncingBall ball){
        this.radius = radius;
        this.speed = speed;
        this.speedX = speedX;
        this.speedY = speedY;
    ...
    public BouncingBall(Field field, int radius, int speed, Color color, double x, double y){
        this.field = field;
        this.radius = radius;
        this.speed = speed;
        double angle = Math.random() * 2 * Math.PI;
    ...
    public BouncingBall(Field field, int radius, int speed, Color color){
        this.field = field;
        this.radius = radius;
        this.speed = speed;
        double angle = Math.random() * 2 * Math.PI;
...
// В противном случае - активный поток заснѐт
                field.canMove(this);
                //"Конструктор"
                if((x + speedX >= 60 + radius && x + speedX <= 140 - radius && y + speedY >= 60 && y + speedY <= 60 + speedY && speedY > 0 ||
                        x + speedX >= 60 + radius && x + speedX <= 140 - radius && y + speedY <= 140 && y + speedY >= 140 + speedY && speedY < 0) ||
                        (y + speedY >= 60 + radius && y + speedY <= 140 - radius && x + speedX >= 60 && x + speedX <= 60 + speedX && speedX > 0 ||
                                y + speedY >= 60 + radius && y + speedY <= 140 - radius && x + speedX <= 140 && x + speedX >= 140 + speedX && speedX < 0)){
                    getBouncingBall(this);
                    field.addBall(radius, speed, color, 100, 100);
                }
                //"Деструктор"
                if(x + speedX >= field.getWidth() - 140 + radius && x + speed <= field.getWidth() - 60 - radius && y + speedY >= field.getHeight() - 140 + radius && y + speedY <= field.getHeight() - 60 - radius) {
                    return;
                }
                //"Телепорт"
                if(x + speedX >= field.getWidth() - 140 + radius && x + speed <= field.getWidth() - 60 - radius && y + speedY >= 60 + radius && y + speedY <= 140 - radius){
                    field.addBall(radius,speed, color);
                    return;
                }
                if (x + speedX <= radius) {
